...
i_la1=[sekilci(['yla','ylə'],1,'0',i_la1)]
i_daki=[sekilci(['dakı','dəki'],1,'0',i_daki)]
for pol in i_cem:
    pol.sonra=[i_men,i_hal,i_la,i_daki]
for pol in i_men:
    if pol.adi == mens1 or pol.adi == mens4:
        pol.sonra=[i_hal,i_la,i_daki]
        continue
...
for pol in i_hal:
    if pol.adi==hal3 or pol.adi==hal2 or pol.adi==hal4:
        pol.sonra=[[sekilci(['dır','dir','dur','dür'],3,'0',sex3),sekilci(['dır','dir','dur','dür'],3,'0',sex6)]]
    else:
        pol.sonra=[]

n_SpecificNoun=[i_cem,i_men,i_hal,i_la,i_daki]